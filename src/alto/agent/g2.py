...
class G2Agent(DataSourceAgent):
    ...
    def _do_query(self, retry=3):
        ...
        try:
            ...
            latest_ts = tsrange.json()[-1]
            snapshot = requests.get(urljoin(self.base_uri, SNAPSHOT_API.format(latest_ts[1])))
            if not snapshot.ok:
                self.token = None
            ...
            if retry > 0:
                ...
                logging.debug(e)
                logging.debug('Retry the query ({})'.format(e, retry))
                return self._do_query(retry=retry-1)
        return latest_ts[0], self.snapshot
